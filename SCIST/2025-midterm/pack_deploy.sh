...
skip_dirs=("sol" "deploy" "shared" "_memo" "__pycache__" "Readmd.md")
...
for p in $(ls -d */); do
    if ! should_skip "$p"; then
        ...
        cp -r "$p" "$tmp_dir"
        cleanup $tmp_dir
        find $tmp_dir -type f -exec sed -i '' -e 's/SCIST{[^}]*}/SCIST{fakeflag}/g' {} \;
        zip -r "shared/${p%/}-shared.zip" "$tmp_dir" -x ".DS_Store" "exploit.py" "**/__pycache__" "**/.ropeproject"
        rm -rf "$tmp_dir"
    fi
done
